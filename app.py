...
st.set_page_config(page_title="V17.5 終極指揮中心", layout="wide")
...
def calculate_v175_logic(df_slice, prev_row):
    """V17.5 核心邏輯：四階色彩與整數權重"""
...
def analyze_v175(ticker):
    try:
        ...
        for i in range(4, -1, -1):
            idx = len(df) - 1 - i
            _, _, _, t_s, _, _ = calculate_v175_logic(df.iloc[:idx+1], df.iloc[idx-1])
            history.append(int(t_s))
            
        f1, f2, f3, total, cmd, bg = calculate_v175_logic(df, df.iloc[-2])
        return {"p": df.iloc[-1]['Close'], "s": total, "cmd": cmd, "bg": bg, "f1": f1, "f2": f2, "f3": f3, "h": history}
    except: return None

# --- 4. 11 個完整戰術板塊配置 (新增量子、獨立光通訊) ---
SECTORS = {
...
if search_ticker:
    res = analyze_v175(search_ticker)
    if res:
...
st.divider()
st.caption(f"偵察時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V17.5 戰略復歸最終版")

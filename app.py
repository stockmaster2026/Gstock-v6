...
# --- 1. 核心數據獲取 (解決 YFRateLimitError 與 ValueError) ---
@st.cache_data(ttl=3600) # 快取 1 小時，避免頻繁請求被鎖
def get_v32_5_safe_data(ticker):
    try:
        # 抗封鎖戰術：模擬真人隨機停頓 2-4 秒
        time.sleep(random.uniform(2, 4)) 
        
        # 抓取數據 (稍微抓長一點確保計算指標不噴 ValueError)
...
with st.sidebar:
    ...
    if st.button("♻️ 強制刷新 (解除封鎖用)"):
        ...
        st.rerun()
    st.divider()
    st.info("V32.5 版權重：A1(25%) A2(30%) A3(45%)")

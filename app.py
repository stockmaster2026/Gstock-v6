...
def analyze_stock(ticker, spy_df):
    try:
        ...
        for i in range(-4, 1):
            ...
            a3 = v_s + r_s
            a3w = round(a3 * 0.4, 1)
            hist_scores.append(round(a1w + a2w + a3w, 1))
            
        return {"tk": ticker, "sc": hist_scores[-1], "bg": "#dcfce7" if hist_scores[-1] >= 9 else "#f1f5f9", 
...
if user_query:
    ...
    if q_res:
        st.sidebar.markdown(f'<div class="stock-row" style="background-color: {q_res["bg"]}; border-left: 6px solid #4f46e5;"><b>🔍 {q_res["tk"]} — ${q_res["pr"]}</b><div class="score-text">{q_res["sc"]} <small>分</small></div><hr><small>A1:{q_res["a1"]} | A2:{q_res["a2"]} | A3:{q_res["a3"]}</small><br><small>5日 AWI: {q_res["hist"]}</small></div>', unsafe_allow_html=True)

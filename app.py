...
st.set_page_config(page_title="V19.6 終極指揮中心", layout="wide")
...
def calculate_v196_logic(df_slice, prev_row):
...
def analyze_v196(ticker):
    try:
        ...
        for i in range(5, -1, -1):
            idx = len(df) - 1 - i
            _, _, _, t_s, _, _ = calculate_v196_logic(df.iloc[:idx+1], df.iloc[idx-1])
            history.append(int(t_s))
        f1, f2, f3, total, cmd, bg = calculate_v196_logic(df, df.iloc[-2])
...
def render_card(t, data):
    ...
    <div class="card-box" style="background-color: {data['bg']};">
        ...
        <div style="text-align:left; background:rgba(0,0,0,0.3); padding:8px; border-radius:6px; margin:10px 0; font-size:12px; line-height:1.6;">
            ...
            <div>🔥 主力(F3)(30%): <b>{data['f3']}/10</b></div>
...
if search_ticker:
    res = analyze_v196(search_ticker)
    if res:
        st.sidebar.markdown(render_card(search_ticker, res), unsafe_allow_html=True)
    elif search_ticker != "":
        st.sidebar.error(f"⚠️ 無法抓取 '{search_ticker}' 資料。")

# --- 7. UI 渲染：主版圖 ---
st.title("🛡️ 三維戰略指揮中心 V19.6")
...
st.divider()
st.caption(f"數據時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V19.6 終極修復定錨版")

...
def calculate_apex_score(ticker, df, spy_df):
    ...
    try:
        ...
        elif vol_ratio < 0.5: a3_score += 4     # 窒息量
...
st.markdown(f"最後掃描：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if spy_df is not None:
    ...
    if all_results:
        ...
        st.dataframe(final_df.style.applymap(lambda x: 'background-color: #ffa500; color: black' if '☀️' in str(x) else '', subset=['AWI 指標']), use_container_width=True)

    # 側邊欄實戰診斷
    st.sidebar.header("🎯 核心建議")
    pl_data = next((item for item in all_results if item["代碼"] == "PL"), None)
    if pl_data:
        st.sidebar.success(f"PL 狀態：{pl_data['AWI 指標']}。維持 $31.00 支撐。")

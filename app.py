...
def analyze_v11(ticker):
    try:
        ...
        elif total >= 3: stat, cmd = "⚠️ 減碼警戒", "減 1/2 / 獲利了結"
...
# --- UI 渲染 ---
st.set_page_config(page_title="V11.0 終極穩定版", layout="wide")

# --- 側邊欄：獨立偵察艙 ---
st.sidebar.title("🕹️ 戰略控制台")
c_ticker = st.sidebar.text_input("🔍 自定義偵察", "").upper()
if c_ticker:
    res = analyze_v11(c_ticker)
    if res:
        ...
        st.sidebar.markdown(f"""<div style="background-color:rgba(0,0,0,0.5); padding:8px; border-radius:6px; text-align:center;"><p style="font-size:11px; color:#CCC; margin:0;">滾動五日軌跡</p><p style="font-size:15px; font-weight:bold; color:#00FF00; margin:0;">{h_str}</p></div>""", unsafe_allow_html=True)
        st.sidebar.metric("當前價格", f"${res['p']:.2f}", f"{res['ch']:.1f}%")

st.title("🛡️ 三維戰略指揮中心 V11.0")
st.markdown("### 📊 滾動五日軌跡 [ T-4 | T-3 | T-2 | T-1 | 今日 ]")
st.divider()
...
SECTORS = {
    ...
    "🛰️ 國防安全": ["KTOS", "BBAI", "RCAT", "CRCL"], # 已加入 CRCL
    "📡 無線通訊": ["ONDS", "QCOM", "TMUS"]
}
    ...
    for j, t in enumerate(tickers):
        ...
        with grid[j % 4]:
            if res:
                ...
                st.markdown(f"""<div style="background-color:{bg}; padding:10px; border-radius:10px; text-align:center; color:white; border:1px solid #555; min-width:200px;"><h3 style="margin:0; font-size:15px;">{t}</h3><p style="font-size:18px; font-weight:bold; margin:2px 0;">${res['p']:.2f}</p><div style="background-color:rgba(0,0,0,0.5); padding:5px; border-radius:5px; margin:5px 0;">{traj_html}</div><hr style="margin:5px 0; opacity:0.3;"><p style="font-size:13px; font-weight:bold; color:#FFD700; margin:0;">{res['s']} Pts | {res['stat']}</p><p style="font-size:11px; margin:0;"><b>{res['cmd']}</b></p></div>""", unsafe_allow_html=True)

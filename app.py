...
# --- 1. 環境定錨 ---
st.set_page_config(page_title="V28.2 巔峰雙軌指揮中心", layout="wide")
...
for sector, tickers in SECTORS.items():
    ...
    st.markdown(f'<div class="h-wrapper">{"".join([render_v28_card(t, d) for t, d in cols_data])}</div>', unsafe_allow_html=True)

st.caption(f"數據更新: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V28.2 靈魂回歸版")

...
# --- 0. UI 終極視覺對位規範 (5日密集化、全板塊回歸) ---
st.set_page_config(layout="wide", page_title="雙軌指揮中心 V32.9.50")
...
st.markdown("""
    ...
    .main-card { color: #333; min-height: 270px; }
    ...
    .data-row { 
        font-size: 0.75rem; font-weight: 900; line-height: 1.4; margin-top: 5px; 
        border-top: 1px solid rgba(128,128,128,0.4); padding-top: 5px; text-align: center;
    }
    
    /* 陣列密集化核心：大幅收縮間距，確保5個數據全數呈現 */
    .num-align {
        font-family: 'Courier New', monospace; background-color: rgba(0,0,0,0.25);
        padding: 6px 1px; /* 上下維持，左右極致密集 */
        border-radius: 4px; font-size: 0.88rem; font-weight: 900;
        color: #00ff00 !important; white-space: pre; margin-top: 6px; 
        line-height: 1.3; display: block; width: 100%; text-align: center;
        letter-spacing: -0.8px; /* 大幅收縮字元間距 */
    }

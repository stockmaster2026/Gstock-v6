...
# --- 0. UI 終極視覺規範 (PS點零、SB軌跡、權重+名稱全顯示) ---
st.set_page_config(layout="wide", page_title="雙軌指揮中心 V32.9.47")
...
st.markdown("""
    ...
    /* 數據行：權重與名稱強制顯示，字體加粗密集 */
    .data-row { 
        font-size: 0.8rem; font-weight: 900; line-height: 1.5; margin-top: 5px; 
...
def render_card(tkr, d):
    ...
    <div class="main-card {d['bg']}">
        ...
        <div class="data-row">
            技術(30%):<b>{d['x1']}</b> / 冠軍(40%):<b>{d['x2']}</b> / 主力(30%):<b>{d['x3']}</b>
        </div>
        <div class="num-align {"light" if "bg-observe" in d["bg"] else ""}">
            PS: {d["h_ps"]}<br>SB: {d["h_sb"]}
        </div>

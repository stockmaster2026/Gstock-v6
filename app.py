...
# --- 1. 系統環境與針對 iPad 的 CSS 終極優化 ---
st.set_page_config(page_title="V18.0 終極指揮中心", layout="wide")
...
def calculate_v18_logic(df_slice, prev_row):
    """V18.0: RSI(50-75) & 1.5% Buffer & 四階燈號"""
    ...
    is_washout = (p < prev_row['Close'] and vol_ratio < 0.8) # 價跌量縮 = 鑽石洗盤
    ...
    # 四階燈號指令
    if f2_std == 0:
        cmd, bg = "❌ 嚴禁介入：格局破壞(全速撤退)。", "#641E1E" # 深紅
    else:
        if total >= 9.5: cmd, bg = "🔥 三維共振：最強攻擊(重倉持有)。", "#1E4620" # 深綠
        elif total >= 6.5: cmd, bg = "💪 蓄勢轉強：跡象現身(回測即買)。", "#326432" # 淺綠
        elif total >= 4.5: cmd, bg = "💎 鑽石洗盤：主力未走(縮量待發)。" if is_washout else "💤 極致洗盤：格局尚在(縮量待爆)。", "#64641E" # 淺紅/黃
...
def analyze_v18(ticker):
    try:
        ...
        history = []
        for i in range(5, -1, -1): # 抓 6 天來比對
            idx = len(df) - 1 - i
            _, _, _, t_s, _, _ = calculate_v18_logic(df.iloc[:idx+1], df.iloc[idx-1])
            history.append(int(t_s))
            
        f1, f2, f3, total, cmd, bg = calculate_v18_logic(df, df.iloc[-2])
        
        # 動能箭頭判定 (今日 vs 昨日)
        trend = "⬆️" if history[-1] > history[-2] else "⬇️" if history[-1] < history[-2] else "—"
        
        return {"p": df.iloc[-1]['Close'], "s": total, "trend": trend, "cmd": cmd, "bg": bg, "f1": f1, "f2": f2, "f3": f3, "h": history[1:]}
    except: return None
...
if search_ticker:
    ...
    if res:
        ...
        <div style="background-color:{res['bg']}; padding:15px; border-radius:10px; color:white; border:1px solid #555;">
            ...
            <p style="font-size:20px; font-weight:bold; color:#FFD700; margin:0;">{res['s']} Pts</p>
...
st.divider()
st.caption(f"數據時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V18.0 終極定錨完全體")

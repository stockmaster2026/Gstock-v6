...
# --- 1. 介面配置 ---
st.set_page_config(page_title="V31.9 巔峰指揮中心", layout="wide")
...
def render_card(t, df):
    ...
    avg_f1 = sum(f1_list) / 5
    avg_f2 = sum(f2_list) / 5
    avg_f3 = sum(f3_list) / 5
...
if search_ticker:
    with st.sidebar:
        ...
        if df_m is not None: st.sidebar.markdown(render_card(search_ticker, df_m), unsafe_allow_html=True)
        else: st.sidebar.error("查無資料")

...
# --- 1. 環境定錨 ---
st.set_page_config(page_title="V21.0 冠軍指揮中心", layout="wide")
...
# --- 2. 核心邏輯 (🥇 F2 定錨) ---
def calculate_v21_logic(df_slice, prev_row):
...
def analyze_v21(ticker):
    try:
        t_str = str(ticker).strip().upper()
        t_obj = yf.Ticker(t_str)
        # 關鍵修正：改用 2y 數據，給 MA200 留出充足空間
        df = t_obj.history(period="2y", timeout=15)
        ...
        # 只在最後階段 dropna，保留當前價格
        df_clean = df.dropna()
        if df_clean.empty: return {"status": "error"}
        
        history = []
        for i in range(5, -1, -1):
            idx = len(df_clean) - 1 - i
            _, _, _, t_s, _, _ = calculate_v21_logic(df_clean.iloc[:idx+1], df_clean.iloc[idx-1])
            history.append(int(t_s))
            
        f1, f2, f3, total, cmd, bg = calculate_v21_logic(df_clean, df_clean.iloc[-2])
...
def render_card(t, data):
    if data["status"] == "error":
        return f'<div class="card-box" style="background-color:#222; opacity:0.6;"><h3 style="margin:0;">{t}</h3><p style="font-size:12px; margin-top:20px;">📡 偵察信號中斷...<br>請確認代碼(IONQ而非iOS)</p></div>'
...
if search_ticker:
    res = analyze_v21(search_ticker)
    st.sidebar.markdown(render_card(search_ticker, res), unsafe_allow_html=True)

st.title("🛡️ 三維戰略指揮中心 V21.0")
st.markdown("### 📊 全域對帳：F2(50%) | F3(30%) | F1(20%) [終極全功能版]")
...
for sector, tickers in SECTORS.items():
    ...
    results = [ (t, analyze_v21(t)) for t in tickers ]
...
st.caption(f"數據最後偵察: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V21.0 終極定錨")

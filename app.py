...
# --- 2. UI 渲染 ---
st.set_page_config(page_title="V12.8 全戰場復歸版", layout="wide")
...
c_ticker = st.sidebar.text_input("🔍 偵察代碼 (如 IONQ)", "").upper()

if c_ticker:
    res = analyze_v128(c_ticker)
    if res:
        st.sidebar.markdown(f"## 🎯 {c_ticker} 報告")
        st.sidebar.error(f"📍 戰略狀態：{res['stat']}")
        st.sidebar.warning(f"👉 操作建議：{res['cmd']}")
        st.sidebar.info(f"🧱 **冠軍結構(F2-50%)**: {res['f2']}/5\n\n🔥 **主力籌碼(F3-30%)**: {res['f3']}/3\n\n✅ **技術指標(F1-20%)**: {res['f1']}/2")
        h = res['history']
...
st.title("🛡️ 三維戰略指揮中心 V12.8")
st.markdown("### 📊 全版圖對帳：F2 結構(50%) | F3 籌碼(30%) | F1 指標(20%)")

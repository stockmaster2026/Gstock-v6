...
# --- 1. 環境定錨 ---
st.set_page_config(page_title="V26.0 冠軍指揮中心", layout="wide")
...
def calculate_v26_logic(df_slice, prev_row):
    try:
        ...
        if f2_std == 0:
            ...
            elif total >= 4.5: cmd, bg = "💎 鑽石洗盤：主力未走。" if is_washout else "💤 結構整理：等待表態。", "#64641E"
...
def analyze_v26(ticker):
    t_str = str(ticker).strip().upper()
    try:
        data_obj = yf.Ticker(t_str)
        df = data_obj.history(period="2y", interval="1d", timeout=12)
        if df.empty: return {"status": "error"}
        
        # 🌟 重要修正：處理 yfinance 的多層索引 Bug
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
            
        # 🌟 重要修正：排除成交量為 0 的非交易日雜訊 (專治 IONQ)
        ...
        if len(df) < 10: return {"status": "error"}
        ...
        trend = "⬆️" if len(history) >= 2 and history[-1] > history[-2] else "—"
...
def render_card(t, data):
    if data["status"] == "error":
        return f'<div class="card-box" style="background-color:#222;"><h3 style="margin:0;">{t}</h3><p style="margin-top:20px;">📡 信號受干擾<br>請點選左側刷新</p></div>'
    ...
    <div class="card-box" style="background-color: {data['bg']};">
        ...
        <div style="background-color:black; color:#0f0; padding:5px; border-radius:6px; font-family:monospace; font-size:16px; margin-bottom:10px; font-weight:bold;">{" | ".join([str(x) for x in data['h']])}</div>
...
for sector, tickers in SECTORS.items():
    ...
    results = [ (t, analyze_v26(t)) for t in tickers ]
    results.sort(key=lambda x: (x[1].get('s', -1) if x[1]['status']=='ok' else -2), reverse=True)
    st.markdown(f'<div class="h-wrapper">{"".join([render_card(t, d) for t, d in results])}</div>', unsafe_allow_html=True)

st.caption(f"數據最後更新: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V26.0 冠軍鎖定版")

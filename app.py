...
# --- UI 渲染 ---
st.set_page_config(page_title="V12.0 終極穩定版", layout="wide")

# --- 側邊欄 ---
st.sidebar.title("🕹️ 戰術控制台")
c_ticker = st.sidebar.text_input("🔍 偵察代碼", "").upper()
if c_ticker:
    res = analyze_v12(c_ticker)
    if res:
        ...
        st.sidebar.markdown(f"""<div style="background-color:#111; padding:10px; border-radius:8px; text-align:center; color:#0f0; font-size:18px; font-weight:bold; border:1px solid #333;">{h_str}</div>""", unsafe_allow_html=True)
        st.sidebar.metric("價格", f"${res['p']:.2f}", f"{res['ch']:.1f}%")

st.title("🛡️ 三維戰略指揮中心 V12.0")
st.markdown("### 📊 指標對帳：F2 冠軍結構(50%) | F3 主力籌碼(30%) | F1 技術指標(20%)")
st.divider()
    ...
    for j, t in enumerate(tickers):
        ...
        with grid[j % 4]:
            if res:
                ...
                # 🌟 使用最簡潔、不報錯的 HTML 格式
                st.markdown(f"""
                <div style="background-color:{bg}; padding:15px; border-radius:12px; text-align:center; color:white; border:1px solid #555; min-height:340px;">
                    ...
                    <div style="text-align:left; background:rgba(0,0,0,0.3); padding:8px; border-radius:6px; margin:10px 0; font-size:11px;">
                        <div style="margin-bottom:2px;">🧱 冠軍結構(F2): <b>{res['f2']}</b></div>
                        <div style="margin-bottom:2px;">🔥 主力籌碼(F3): <b>{res['f3']}</b></div>
                        <div>✅ 技術指標(F1): <b>{res['f1']}</b></div>
                    </div>

                    <div style="background-color:#000; padding:8px; border-radius:6px; margin:10px 0; border:1px solid #444; color:#0f0; font-weight:bold; font-size:14px; letter-spacing:2px;">
                        {h[0]} | {h[1]} | {h[2]} | {h[3]} | {h[4]}
                    </div>
                    
                    <hr style="margin:10px 0; opacity:0.3;">
                    <p style="font-size:14px; font-weight:bold; color:#FFD700; margin:0;">{res['s']} Pts | {res['stat']}</p>
                    <p style="font-size:11px; margin-top:4px;"><b>{res['cmd']}</b></p>
                </div>

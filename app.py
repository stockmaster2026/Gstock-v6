...
# --- 1. 系統環境與針對 iPad 的 CSS 優化 ---
st.set_page_config(page_title="V14.0 終極指揮中心", layout="wide")
...
def calculate_v140_logic(df_slice, prev_row):
    """V14.0 核心邏輯：10分制共同分母"""
...
def analyze_v140(ticker):
    try:
        ...
        for i in range(4, -1, -1):
            idx = len(df) - 1 - i
            _, _, _, t_s, _ = calculate_v140_logic(df.iloc[:idx+1], df.iloc[idx-1])
            history.append(t_s)
            
        f1, f2, f3, total, cmd = calculate_v138_logic(df, df.iloc[-2]) if 'calculate_v138_logic' in globals() else calculate_v140_logic(df, df.iloc[-2])
...
for sector, tickers in SECTORS.items():
    ...
    for t in tickers:
        ...
        if data:
            ...
            # 🌟 核心修復：使用 &dollar; 替代 $ 符號，並將所有 HTML 包裹在單一字串中
            card_html = f"""
            ...
            """
            cards_accumulator += card_html
            
    # 將整個板塊一次性渲染，確保橫向滑動容器閉合正確
    final_row_html = f'<div class="h-wrapper">{cards_accumulator}</div>'
    st.markdown(final_row_html, unsafe_allow_html=True)

st.divider()
st.caption(f"偵察時間: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | V14.0 終極修復定錨版")

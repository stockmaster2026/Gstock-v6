...
# --- UI 渲染 ---
st.set_page_config(page_title="V12.3 終極指揮中心", layout="wide")
...
if c_ticker:
    res = analyze_v123(c_ticker)
    if res:
        st.sidebar.markdown(f"## 🎯 {c_ticker} 報告")
        st.sidebar.error(f"📍 戰略狀態：{res['stat']}")
        st.sidebar.warning(f"👉 操作建議：{res['cmd']}") # 強制顯示
        
        st.sidebar.info(f"""
        🧱 **冠軍結構 (F2)**: {res['f2']}/5 Pts
...
# --- 主頁面區域 ---
st.title("🛡️ 三維戰略指揮中心 V12.3")
st.markdown("### 📊 全域對帳：F2 冠軍結構(50%) | F3 主力籌碼(30%) | F1 技術動能(20%)")
st.divider()
...
SECTORS = {
    ...
    "🛰️ 國防安全": ["KTOS", "BBAI", "RCAT", "CRCL"], # CRCL 正式入榜
    "📡 無線通訊": ["ONDS", "QCOM", "TMUS"]
}
    ...
    for j, t in enumerate(tickers):
        ...
        with grid[j % 4]:
            if res:
                ...
                # 🌟 視覺終極修復：使用最穩定的模板化輸出，確保 100% 顯示
                # 1. 頂部：代碼與價格
                # 2. 中間：今日分項對帳 (白話文)
                # 3. 底部：五日軌跡與操作指令
                card_html = f"""
                <div style="background-color:{bg}; padding:15px; border-radius:12px; text-align:center; color:white; border:1px solid #555; min-height:350px;">
                    <h3 style="margin:0; font-size:16px;">{t}</h3>
                    <p style="font-size:22px; font-weight:bold; margin:5px 0;">${res['p']:.2f}</p>
                    
                    <div style="text-align:left; background:rgba(0,0,0,0.3); padding:8px; border-radius:6px; margin:10px 0; font-size:11px; line-height:1.5;">
                        <div>🧱 冠軍結構(F2): <b>{res['f2']}</b></div>
                        <div>🔥 主力籌碼(F3): <b>{res['f3']}</b></div>
                        <div>✅ 技術動能(F1): <b>{res['f1']}</b></div>
                    </div>
                    ...
                    <p style="font-size:11px; margin-top:5px; line-height:1.2;"><b>{res['cmd']}</b></p>
                </div>
                """

import streamlit as st
import yfinance as yf
import pandas as pd
import pandas_ta as ta
import numpy as np
from datetime import datetime, timedelta

# --- 1. 系統核心設定 ---
st.set_page_config(page_title="Apex Ambush V32.5", layout="wide")

# 定義觀察清單 (妳最關心的 11 支標的)
WATCH_LIST = ["AAOI", "PL", "LUNR", "TSLA", "NVDA", "TSEM", "CRDO", "MSFT", "GOOGL", "META", "AAPL"]

# --- 2. 核心評分引擎：V26.5 巔峰埋伏邏輯 ---
def calculate_apex_score(ticker, df, spy_df):
    """
    進化點：
    A1: 趨勢(20%) | A2: 構造(40%) | A3: 能量(40%)
    導入 RS 相對強度與 AWI 天氣指標
    """
    try:
        if len(df) < 50: return None
        
        latest = df.iloc[-1]
        prev = df.iloc[-2]
        
        # A_1: 趨勢對齊 (20%) - MACD 雙線與零軸關係
        a1_score = 0
        # 抓取 MACD 欄位 (支援 pandas_ta 預設命名)
        macd_col = [c for c in df.columns if 'MACD_' in c and 'h' not in c and 's' not in c][0]
        if latest[macd_col] > 0: a1_score = 10
        elif latest[macd_col] > prev[macd_col]: a1_score = 6
        
        # A_2: 構造錨定 (40%) - 均線壓縮 (系統靈魂)
        ma_list = [latest['MA10'], latest['MA20'], latest['MA50']]
        ma_std = np.std(ma_list)
        ma_avg = np.mean(ma_list)
        compression = (ma_std / ma_avg) * 100
        
        a2_score = 0
        if compression < 1.5: a2_score = 10     # 極度壓縮 (起漲靈魂)
        elif compression < 3.0: a2_score = 8    # 標準壓縮
        elif compression < 5.0: a2_score = 4    # 稍嫌鬆散
        
        # A_3: 能量活化 (40%) - 成交量 & RS 隱形推力
        a3_score = 0
        avg_vol = df['Volume'].tail(10).mean()
        vol_ratio = latest['Volume'] / avg_vol
        
        # 能量判定 (窒息或攻擊)
        if vol_ratio > 1.3: a3_score += 5       # 攻擊量
        elif vol_ratio < 0.5: a3_score += 4     # 窒息量 (埋伏)
        
        # RS 強度判定 (對比大盤 SPY 20日漲幅)
        stock_ret = (latest['Close'] / df.iloc[-20]['Close']) - 1
        spy_ret = (spy_df.iloc[-1]['Close'] / spy_df.iloc[-20]['Close']) - 1
        rs_strength = stock_ret - spy_ret
        
        if rs_strength > 0.02: a3_score += 5    # 強於大盤 2% 以上 (有大戶撐腰)
        elif rs_strength > 0: a3_score += 3
        
        # 總分計算
        total = (a1_score * 0.2) + (a2_score * 0.4) + (a3_score * 0.4)
        
        # AWI 天氣系統轉換
        if total >= 9.0: weather, icon = "噴發態", "🎆"
        elif total >= 7.0: weather, icon = "強勢態", "☀️"
        elif total >= 5.0: weather, icon = "整理態", "☁️"
        else: weather, icon = "危險態", "🌫️"
        
        # F1 戰略過濾器 (動態彈性修正)
        # 如果 A3 能量夠強 (a3_score >= 8)，放寬乖離率限制至 5%
        buffer_limit = 0.05 if a3_score >= 8 else 0.015
        dist_to_ma20 = abs(latest['Close'] - latest['MA20']) / latest['MA20']
        f1_pass = dist_to_ma20 <= buffer_limit
        
        return {
            "代碼": ticker,
            "AWI 指標": f"{icon} {weather}",
            "Apex 總分": round(total, 1),
            "F1 過濾": "✅ PASS" if f1_pass else "❌ WAIT",
            "均線壓縮度": f"{round(compression, 2)}%",
            "相對強度(RS)": f"{round(rs_strength*100, 2)}%",
            "成交量比": round(vol_ratio, 2),
            "現價": round(latest['Close'], 2)
        }
    except Exception as e:
        return {"代碼": ticker, "AWI 指標": "Error", "Apex 總分": 0}

# --- 3. 數據抓取與處理 (V32 完整邏輯) ---
def get_data(ticker):
    df = yf.download(ticker, period="3mo", interval="1d", progress=False)
    if df.empty: return None
    
    # 計算 V32 技術指標
    df['MA10'] = ta.sma(df['Close'], length=10)
    df['MA20'] = ta.sma(df['Close'], length=20)
    df['MA50'] = ta.sma(df['Close'], length=50)
    macd = ta.macd(df['Close'])
    df = pd.concat([df, macd], axis=1)
    return df

# --- 4. Streamlit 介面呈現 ---
st.title("🛰️ Apex Ambush V32.5 巔峰埋伏系統")
st.markdown(f"**最後更新時間：** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (美股實時監控)")

# 獲取基準數據
spy_df = get_data("SPY")

if spy_df is not None:
    all_results = []
    
    # 建立橫向進度條
    progress_bar = st.progress(0)
    
    for i, t in enumerate(WATCH_LIST):
        stock_df = get_data(t)
        if stock_df is not None:
            res = calculate_apex_score(t, stock_df, spy_df)
            if res:
                all_results.append(res)
        progress_bar.progress((i + 1) / len(WATCH_LIST))

    # 顯示核心數據表
    final_df = pd.DataFrame(all_results)
    
    # 使用 Style 讓表格更易讀
    def color_awi(val):
        if '🎆' in str(val): return 'background-color: #ff4b4b; color: white'
        if '☀️' in str(val): return 'background-color: #ffa500; color: black'
        if '🌫️' in str(val): return 'background-color: #d3d3d3; color: gray'
        return ''

    st.subheader("📊 全域戰略監控看板")
    st.dataframe(final_df.style.applymap(color_awi, subset=['AWI 指標']), use_container_width=True)

    # --- 5. 側邊欄：當前持倉診斷 ---
    st.sidebar.header("🎯 核心診斷建議")
    
    # 自動抓取 PL 與 LUNR 狀態
    pl_status = next((item for item in all_results if item["代碼"] == "PL"), None)
    lunr_status = next((item for item in all_results if item["代碼"] == "LUNR"), None)
    
    if pl_status:
        st.sidebar.markdown(f"**PL (持倉建議):**")
        if "☀️" in pl_status["AWI 指標"]:
            st.sidebar.success(f"目前分數 {pl_status['Apex 總分']} (☀️)。量縮洗盤中，守住 $31.00，不建議賣出。")
        else:
            st.sidebar.warning(f"PL 跌出強勢區，目前分數: {pl_status['Apex 總分']}")

    if lunr_status:
        st.sidebar.markdown(f"**LUNR (持倉建議):**")
        if "🌫️" in lunr_status["AWI 指標"]:
            st.sidebar.error(f"警告：分數僅 {lunr_status['Apex 總分']} (🌫️)。能量崩潰且 RS 走弱，建議果斷撤資。")

st.sidebar.write("---")
st.sidebar.info("V32.5 更新日誌：\n1. A3 權重調升至 40%\n2. 加入 RS 相對強度過濾\n3. 動態 F1 Buffer (1.5% -> 5%)")


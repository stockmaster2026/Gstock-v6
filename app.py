...
class MasterTraderSystem:
    ...
    def analyze(self, d):
        # F1: 嚴格執行 MACD > 0, RSI > 50, MACD柱狀 > 0
    ...
    def get_stage_text(self, f1, f2, f3_res, d):
        if f3_res == "破位":
            return f"【空頭排列】已跌破生命線。目前下行趨勢明顯(RSI:{d['rsi']:.1f})，嚴禁入場買進。"
        if not f1:
            return "【動能背離】雖然仍在生命線附近，但指標(MACD/RSI)已轉弱或低於50分界點，多頭熄火。"
        if not f2:
            return "【高檔派發】指標雖強但型態不佳(長上影線/連黑)，顯示賣壓沉重，小心誘多陷阱。"
        return "【多頭攻擊】三指標共振且站穩生命線。結構健康，屬於標準的多頭續攻階段。"

# --- UI 介面 ---
st.title("🛡️ 11 大板塊「三輪過濾」決策系統")
st.write(f"當前系統時間：2026年3月17日")

system = MasterTraderSystem()

if st.button("啟動 F1-F3 板塊實時分析"):
    results = []
    with st.spinner('正在抓取全球 11 大板塊數據...'):
        for name, ticker in system.sectors.items():
            ...
            if data:
                ...
                results.append({
                    ...
                    "F3": f"{f3_icon} {f3_res}", "階段判讀 (白話文)": stage
                })
        
        if results:
            st.table(pd.DataFrame(results))
        else:
            st.error("數據抓取失敗，請檢查網路連接。")

...
if spy_df is not None:
    ...
    if all_results:
        ...
        st.dataframe(final_df.style.applymap(lambda x: 'background-color: #ffa500' if '☀️' in str(x) else '', subset=['AWI 指標']), use_container_width=True)

    st.sidebar.header("🎯 實戰診斷")
    pl_res = next((item for item in all_results if item["代碼"] == "PL"), None)
    if pl_res:
        st.sidebar.success(f"PL 現況：{pl_res['AWI 指標']}。守住 $31.00。")

...
# --- 1. 樣式配置 ---
st.set_page_config(page_title="V31.6 巔峰指揮中心", layout="wide")
...
def render_card(t, df):
    ...
    # --- 白話文狀態判定邏輯 ---
    if v26_5day_total >= 45: 
        bg, cmd = "#1E4620", "🔥 冠軍共振・最強進攻"
    elif v26_5day_total >= 35: 
        bg, cmd = "#2E5A2E", "⚡ 動能蓄勢・準備突破"
    elif v26_5day_total >= 20: 
        bg, cmd = "#64641E", "💤 縮量洗盤・等待表態"
    elif v26_5day_total >= 10:
        bg, cmd = "#461E1E", "⚠️ 弱勢整理・不宜接刀"
    else:
        bg, cmd = "#121212", "❌ 趨勢破壞・避開空頭"

    return f"""
    <div style="background-color: {bg}; border: 1.5px solid #00ffcc; padding: 15px; border-radius: 12px; text-align: center; color: white;">
        <h3 style="margin:0; font-size:18px;">{t} {ic_h[-1]}</h3>
        <div class="current-score" style="margin: 8px 0;">五日累計戰力: {v26_5day_total} / 50</div>
        <p style="font-size:22px; font-weight:bold; margin:5px 0;">&dollar;{p:.2f} <span style="font-size:14px; color:#0f0;">({ch:+.2f}%)</span></p>
        <div style="text-align:left; background:rgba(0,0,0,0.3); padding:10px; border-radius:8px; font-size:11px; line-height:1.6;">
            <div>🥇 結構(F2): <b>{int(f2_now)}/10</b> | 🔥 籌碼(F3): <b>{int(f3_now)}/10</b></div>
            <div>✅ 技術(F1): <b>{int(f1_now)}/10</b> | 🏆 今日分: <b>{v_now}</b></div>
        </div>
...
if search_ticker:
    with st.sidebar:
        ...
        if df_m is not None: st.markdown(render_card(search_ticker, df_m), unsafe_allow_html=True)
        else: st.error("查無資料")

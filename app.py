...
def analyze_v109(ticker):
    try:
        ...
        elif total >= 8: stat, cmd = "✅ 積極建倉", "建立 1/2 倉 / 分批加碼"
...
# --- UI 渲染 ---
st.set_page_config(page_title="V10.9 偵察艙完全體", layout="wide")
...
# --- 主頁面區域 ---
st.title("🛡️ 三維戰略指揮中心 V10.9")
st.markdown("### 📊 全標的滾動五日軌跡 [ T-4 | T-3 | T-2 | T-1 | 今日 ]")
st.divider()
    ...
    for j, t in enumerate(tickers):
        ...
        with grid[j % 4]:
            if res:
                ...
                # 滾動軌跡 HTML (強制一行)
                traj_html = f"""
                <div style="display: flex; justify-content: center; align-items: center; width: 100%; white-space: nowrap;">
                    <span style="font-size: 14px; font-weight: bold; color: #00FF00;">{res['history'][0]}</span>
                    <span style="color: #666; font-size: 9px; margin: 0 3px;">|</span>
                    <span style="font-size: 14px; font-weight: bold; color: #00FF00;">{res['history'][1]}</span>
                    <span style="color: #666; font-size: 9px; margin: 0 3px;">|</span>
                    <span style="font-size: 14px; font-weight: bold; color: #00FF00;">{res['history'][2]}</span>
                    <span style="color: #666; font-size: 9px; margin: 0 3px;">|</span>
                    <span style="font-size: 14px; font-weight: bold; color: #00FF00;">{res['history'][3]}</span>
                    <span style="color: #666; font-size: 9px; margin: 0 3px;">|</span>
                    <span style="font-size: 14px; font-weight: bold; color: #00FF00;">{res['history'][4]}</span>
                </div>
                """
                st.markdown(f"""
                <div style="background-color:{bg}; padding:12px; border-radius:12px; text-align:center; color:white; border:1px solid #555; min-width:210px; margin-bottom:10px;">
                    <h3 style="margin:0; font-size:16px;">{t}</h3>
                    <p style="font-size:20px; font-weight:bold; margin:5px 0;">${res['p']:.2f}</p>
                    <div style="background-color:rgba(0,0,0,0.5); padding:6px 4px; border-radius:6px; margin:8px 0; border:1px solid rgba(255,255,255,0.2);">
                        <p style="font-size:10px; color:#CCC; margin:0 0 3px 0;">滾動五日軌跡</p>
                        {traj_html}
                    </div>

...
def analyze_v106(ticker):
    try:
        ...
        elif total >= 5: stat, cmd = "🟡 試探摸索", "建立 1/4 倉 / 嚴守停損"
...
# --- UI 渲染 ---
st.set_page_config(page_title="V10.6 視覺強化版", layout="wide")

# --- 側邊欄 ---
st.sidebar.title("🕹️ 戰略控制台")
custom_ticker = st.sidebar.text_input("🔍 自定義偵察", "").upper()
if custom_ticker:
    res = analyze_v106(custom_ticker)
    if res:
        st.sidebar.markdown(f"## 🎯 {custom_ticker} 報告")
        st.sidebar.info(f"🧱 F2: {res['f2']} | 🔥 F3: {res['f3']} | ✅ F1: {res['f1']}")
        h_str = " | ".join([str(s) for s in res['history']])
        st.sidebar.code(f"五日軌跡：\n{h_str}", language="text")

# --- 主頁面 ---
st.title("🛡️ 三維戰略指揮中心 V10.6")
st.markdown("### 📊 全標的滾動式五日軌跡 [ T-4 | T-3 | T-2 | T-1 | 今日 ]")
st.divider()
    ...
    for j, t in enumerate(tickers):
        ...
        with grid[j % 4]:
            if res:
                ...
                h_str = " | ".join([str(s) for s in res['history']])
                st.markdown(f"""
                <div style="background-color:{bg}; padding:15px; border-radius:12px; text-align:center; color:white; border:1px solid #555; min-width:200px; margin-bottom:10px;">
                    <h3 style="margin:0; font-size:18px;">{t}</h3>
                    <p style="font-size:22px; font-weight:bold; margin:5px 0;">${res['p']:.2f}</p>
                    <div style="background-color:rgba(0,0,0,0.5); padding:8px; border-radius:6px; margin:10px 0; border:1px solid rgba(255,255,255,0.2);">
                        <p style="font-size:11px; color:#CCC; margin:0 0 4px 0;">滾動五日軌跡 (舊→新)</p>
                        <p style="font-size:15px; font-weight:bold; color:#00FF00; letter-spacing:2px; white-space:nowrap;">{h_str}</p>
                    </div>
                    <hr style="margin:10px 0; opacity:0.3;">
                    <p style="font-size:16px; font-weight:bold; color:#FFD700;">{res['s']} Pts | {res['stat']}</p>
                    <p style="font-size:12px; margin-top:5px;"><b>{res['cmd']}</b></p>
                </div>
